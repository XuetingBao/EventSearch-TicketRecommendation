...
public class TicketMasterAPI {
	...
	private static final String API_KEY = "5huEsMhEA4ZkbcljlSdyYcqQd49Mni1F";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			keyword = URLEncoder.encode(keyword, "UTF-8"); // encode 作用：把空格转成20% "Rick Sun" => "Rick%20Sun"
		...
		try {
			...
			// 用BufferedReader更高效，因为一次取8192个数据
			...
			if(!object.isNull("_embedded")) { // 检查返回的response里有没有这个key
	...
	private List<Item> getItemList(JSONArray events) throws JSONException { // 从ticketMaster返回的一堆数据中，挑出我们想要的
...
//			if(!event.isNull("ranting")) {
//				builder.setRanting(event.getDouble("rating"));
//			}
